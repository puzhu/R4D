...
```{r}
get_sum <- function(num_vec1, num_vec2, error_message = "This function only accepts numeric inputs") {
    if (is.numeric(num_vec1) & is.numeric(num_vec2)) {
        # value to be return if numeric
        sum(num_vec1, num_vec2)
    } else {
        # value that is returned if it is not numeric
        stop(error_message)
    }
...
```{r}
get_sum(num_vec1 = 1:2, num_vec2 = 3:4)
```

```{r}
get_sum(c("a", "b"), num_vec2 = 2:3)
```


```{r}
get_sum(c("a", "b"), num_vec2 = 2:3, error_message = "My fancy message")
```
