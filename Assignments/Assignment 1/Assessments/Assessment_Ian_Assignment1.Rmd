...
3. List the heirarchy of data types for coercion
    + Answer: Character, numeric, integer, logical `r emo::ji("check")`
...
"1" == 1
```
Answer: The numeric 1 is coerced into a character in order to attempt the comparison `r emo::ji("check")`

```{r}
FALSE == 0
```
Answer: The number 0 has a falsey value when compared with a logical `r emo::ji("cross mark")`

This aint JS bruh (Although 0 is falsey in R, coercion trumps it)! FALSE is coerced to 0 (TRUE to 1) yielding 0 == 0 which returns TRUE on the comparison.
...
8. What is the difference between an atomic vector and a list?
    + Answer: A vector must be of the same data type, while a list could be items of different types. `r emo::ji("check")`
...
2. Describe two ways in which you can use RStudio confirm that a package has been loaded into your current R session? (Hint: One of them is a dropdown menu and the other a list of checkboxes)
    ...
    + Answer: Under the Packages tab in the bottom right there is a list of all the packages currently installed (checked if they are loaded) and the Global Environment dropdown also lists all loaded packages. `r emo::ji("check")`
...
10. The result from `row_number()` and `top_n()` are the same, while that from `dense_rank()` is different. Why?
    ...
    + See Solution! Dense_rank breaks ties by assigning tied values the same ranks while row_number breaks ties by assigning the first tied value it encounters the first rank and the next rank to the next tied value it encounters.
...
`r emo::ji("man tipping hand")`: This works but I recommend using `top_n()` instead of the arrange + slice combo. Also its not neccessary to ungroup flights at the end (line 201). You have not reassigned a new value to 'flights' and it remain ungrouped.
...
`emo::ji("person gesturing NO")`: It works but negative points for `->`.
...
`r emo::ji("check")`: Works. No need to ungroup flights.
...
flights %>% 
  ...
  slice(1:5)
```
`r emo::ji("man tipping hand")`: Works but use `top_n()` instead.
...
15. The air authority in NY wants to penalize carriers for departure delays. Which of the three metrics (task 1, 3 or 4) would you recommend they use to identify the airlines to penalize. Why?
    ...
    + Answer: The task using comparativeDelays, because it provides a fair basis for comparison between the carriers. `r emo::ji("check")`
...
###My level 3 header `r emo::ji("cross mark")`
