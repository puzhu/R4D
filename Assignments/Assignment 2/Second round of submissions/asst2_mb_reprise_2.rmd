...
test_a %>% 
    select(dep_time) %>% 
    mutate(new_time = strptime(dep_time, format = "%H:%M"))
# test_b
sample(flights$dep_time, size = 10) %>% 
    str_pad(., width = 4, side = "left", pad = 0) %>%
    paste(str_sub(., start = 1, end = 2), str_sub(., start = 3, end = 4), sep = ":") %>% 
    str_sub(start = 6, end = 10) %>% 
    paste0(":00") %>% 
    chron::times(.)
 chron::times(sample(flights$dep_time, size = 10), format = "%H:%M")
```
