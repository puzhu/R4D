...
## takes an integer or character representation and returns a time object. ## requires the chron package
create_time <- function(time_vec) {
  padded_time <- str_pad(time_vec, width = 4, side = "left", pad = 0)
  piece_one <- str_sub(padded_time, start = 1, end = 2)
  piece_two <- str_sub(padded_time, start = 3, end = 4)
  
  paste(piece_one, piece_two, "00", sep = ":") %>% 
    chron::times(.)
} 

create_prediction <- function(time_object) {
  normalized_metric_time <- abs(as.numeric(time_object) / 60 - 19 * 60 - 35)
  max(1 / (1 + exp(-(0.279603750 - 0.001999519 * normalized_metric_time))))
}
  

test_a %>% 
  mutate(is_delay = if_else(dep_delay > 0, 1, 0), 
         new_dep_time = create_time(sched_dep_time)) %>%
  group_by(new_dep_time) %>% 
  summarise(op_sched_dep_time = mean(is_delay, na.rm = T),
            pred_bivar = create_prediction(time_object = new_dep_time)) %>% 
  ggplot() +
  geom_line(size = 1, aes(x = new_dep_time, y = 0.39094), color = 'gray') +
  geom_point(size = 1, mapping = aes(x = new_dep_time, y = op_sched_dep_time, color = 'observed probability')) 

+
  geom_line(data = test_c, mapping = aes(x = new_dep_time, y = pred_bivar, color = 'predicted probability'), size = 1.5) +
