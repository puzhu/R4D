...
$$P(del.) = \frac{1}{1 + e^{-(0.28 - 0.002 * timediff)}}$$

Where $P(del.)$ is the probability of departure delay and $timeidff = |sched. dep. time - 19:35|$ or the number of minutes in either direction between the scheduled departure time of a flight and 7:35 p.m.

```{r message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
## Load needed packages
library(tidyverse); library(nycflights13); library(stringr); library(lubridate); library(ggthemes); library(moments); library(ggalluvial)
```
...
## Next, mutate delay time to a binary i.e. Is it delayed (Y/N)
...
test_c <- test_b %>%
  ...
  group_by(sched_dep_time_metric) %>%
  summarise(op_sched_dep_time = mean(is_delay, na.rm = T),
            pred_bivar = max(1/(1+exp(-(0.279603750 - 0.001999519 * abs(as.numeric(dep_time_object)/60 - 19 * 60 - 35))))))

# test_c
```


```{r echo=FALSE, echo=FALSE, message=FALSE, warning=F, paged.print=TRUE}

test_c %>%
ggplot() +
  geom_line(size = 1, aes(x = sched_dep_time_metric, y = 0.39094), color = 'gray') +
  geom_point(size = 1, mapping = aes(x = sched_dep_time_metric, y = op_sched_dep_time, color = 'observed probability')) +
  geom_line(data = test_c, mapping = aes(x = sched_dep_time_metric, y = pred_bivar, color = 'predicted probability'), size = 1.5) +
  
  ggtitle('Fly in the morning to lower your chance for delay') +
  scale_color_fivethirtyeight(labels = c('observed p', 'predicted p')
...
1. Use functions to do most of your work. Below I have refactored your code using functions to achieve all the calculations you were doing. Take a close look at them and see if you can replicate it elsewhere in your code. This way of coding is much easier to maintain and understand. 
2. Avoid replicating large datasets in multiple variables. R uses your RAM to store the data you create, this means that replicating large datasets using different variables is not a good idea (points to bad coding practice in general) since it will slow everything down. The refactored code below only creates a single summarized tibble to make the plot instead of multiple copies of the large flights tibble.
3. I have also refactored your ggplot code. Go through that to see the differences. Uses some of the stuff we discussed yesterday.
4. Your file currently does not run top to bottom. Fix this.
5. In general, I usually maintain functions that I use for a project separately. Say for instance "helpers.R", which I then load them into my environment using `source("helpers.R")`. This is a convenient way to keep your notebooks concise. I would advise the same here. 

```{r}
## takes a vector with integer times and converts to metric
convert_metric <- function(time_vec) {
  ## get the hour and multiply by 100
  metric_hour <- 100 * trunc(time_vec/100)
  
  ## get the min and scale to 100
  metric_min <- as.numeric(str_sub(as.character(time_vec), -2, -1)) / 60  * 100
  
  ##return the combined value
  metric_hour + metric_min
}

## take a vector with time (in integer format) and convert to minutes
convert_minutes <- function(time_vec) {
  ## hours in minutes
  num_hour <- trunc(time_vec / 100) * 60
  
  ## minutes
  num_min <- as.numeric(str_sub(as.character(time_vec), -2, -1))
  
  ## time in minutes
  num_hour + num_min
}

## given a mid point hour and minute normalize the time
normalize_time <- function(time_mins, mid_hour = 19, mid_min = 35) {
  abs(time_mins - mid_hour * 60 - mid_min)
}

## given a normalized time calculate the predicted value
create_prediction <- function(normalized_time_diff) {
  max(1 / (1 + exp(-(0.279603750 - 0.001999519 * normalized_time_diff))))
}
...
plot_data <- flights %>% 
  group_by(sched_dep_time) %>%
  filter(n() >= 100 & !is.na(dep_delay) == T) %>% 
  ungroup() %>%
  mutate(is_delay = if_else(dep_delay > 0, 1, 0), ## is the flight delayed
         sched_dep_time_metric = convert_metric(sched_dep_time), ## departure time in metric
         sched_dep_time_numeric = convert_minutes(sched_dep_time) ## departure time as numeric
         ) %>% 
  group_by(sched_dep_time_metric) %>% 
  summarise(op_sched_dep_time = mean(is_delay, na.rm = T), ## calculate mean delay
            pred_bivar = create_prediction(normalize_time(sched_dep_time_numeric)) ## calculate the predicted line
            ) %>% 
  ungroup()
...
```{r}
## get the baseline prob
baseline <- sum(flights$dep_delay > 0, na.rm = T) / sum(!is.na(flights$dep_delay))

## draw the plot
ggplot(data = plot_data, mapping = aes(x = sched_dep_time_metric)) + ## x aesthetic is common across all plots so keep it up top
  geom_line(mapping = aes(color = 'gray'), size = 1, y = baseline) +
  geom_point(mapping = aes(y = op_sched_dep_time, color = "red"), size = 1) +
  geom_line(mapping = aes(y = pred_bivar), color = "blue", size = 1.5) +
  scale_color_identity(guide = "legend", label = c('observed p', 'predicted p')) + ## using the identity scale to recognize colors as is
  labs(title = 'Fly in the morning to lower your chance for delay',
      color = "Prob. of delay given sched. dep. time") + ## using labs to label both the legend for color and the plot title
  theme_fivethirtyeight()
```
...
1. Set a baseline probability to build upon
...
4. Clean and adapt the data both so that it makes sense logically and is testable; This will include:
    ...
    b. Replacing categorical data -- such as the name of an airline -- with an observed probability that estimates the added chance for delay associated with any object
...
I'll set a baseline probability by finding $\frac{delayed flights}{total flights}$ : 
...
The baseline probability of delay is roughly `r round(100 * mean(nycflights13::flights$dep_delay > 0, na.rm = T), 0)` %
...
To verify the time numerically, find the correlation with observed probability for each time, then find the max among the outputs. Intuitively, I'm finding the vertical line where what's left looks symmetric to the right.
...
for(i in seq_along(temp_df$sched_dep_time_metric %>% unique())){ ## It's a for loop for every metric time to diff from
...
The max correlation is at 7:35 p.m. To confirm this graphically, I'll redraw the graph of observed probabilities
```{r message=FALSE, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE}
flt_test_01 %>%
  group_by(as.numeric(sched_dep_time_object)) %>%
  filter(n() > 100) %>% ## Limit to sched dep times with over 100 instances
  mutate(is_delay = if_else(dep_delay > 0, 1, 0), ## Turning delay from minutes delayed into a binary
         sched_dep_time_metric = round((sched_dep_time / 100 - trunc(sched_dep_time / 100)) * 500/3 + 100 * (trunc(sched_dep_time / 100))),
         op_sched_dep_metric_left = if_else(sched_dep_time_metric < 1958 + 1/3, mean(is_delay, na.rm = T), NULL),
         op_sched_dep_metric_right = if_else(sched_dep_time_metric >= 1958 + 1/3, mean(is_delay, na.rm = T), NULL)) %>% 
  ungroup() %>%
  select(sched_dep_time_metric, op_sched_dep_metric_left, op_sched_dep_metric_right) %>%
  unique() %>%
  ggplot() + 
  geom_point(mapping = aes(x = sched_dep_time_metric, y = op_sched_dep_metric_left, color = 'op_left')) +
  geom_point(mapping = aes(x = 1958 * 2 - sched_dep_time_metric, y = op_sched_dep_metric_right, color = 'op_right')) +
  ggtitle('Likelihood of departure delay peaks around 7:35 p.m.') +
  scale_color_fivethirtyeight(labels = c('observed p', 'smoothed observed p')
                              ,name = 'Prob. of delay given sched. dep. time') +
  theme_fivethirtyeight() +
...
op_base <- mean(flights$dep_delay > 0, na.rm = T)
```


That baseline is `r round(100 * mean(flights$dep_delay > 0, na.rm = T), 0)` percent. This needs to be graphed
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE}
...
`r colnames(flt_raw %>% select(dest, tailnum, arr_time, day.x, month.x, flight, sched_arr_time, year.x, air_time, arr_delay, dep_time))`

Only one field among those remaining, **wind gust**, is concerning due to the number of n/a entries -- `r    256,391 of 336,776 total. My fear is that these incomplete entries are biased -- that is, we may be telling a story that is based on a sample of the data that does not represent the poopulation.
...
The variables that appear to have significant relationships with dep_delay are: `r colnames(flt_test_03 %>% select(diff_1935h, humid, pressure, visib, op_carr, op_ogn, op_mnf, op_mdl, op_eng))`

### Find correlation with our dependent variable

The analysis could stop with nine variables. However, we should all be concerned with comingled variables. If, for example, we had found instead that both the length of an airplane and the weight of an airplane were associated with departure delays, we might inadvertantly be telling the same story twice. That is, because length and weight are, presumably, associated with each other, including the second variable provides no additional information and may instead be crowding out another useful variable.
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE}
flt_test_03 %>%
...
This brings us to our final five variables: `r colnames(flt_test_03 %>% select(diff_1935h, pressure, visib, op_carr, op_ogn))`
...
signif(coefficients(glm_test), 3)
```


Here is the equation that estimates the probability of a flight being delayed using these five inputs:

$$\frac{1}{1 + e^{-(24.4 - 0.00200 diff_1935h - 0.0230 pressure - 0.0774 visib + 4.19 op_carr + 1.73 op_ogn)}} $$
...
$$\frac{1}{1 + e^{-(0.280 - 0.00200 diff_1935h)}} $$
...
test_00 <- flights %>%
  ...
  ungroup() %>%
  select(sched_dep_time_metric, op_diff1935, diff_1935h, carrier, is_delay, origin, pressure, visib) %>%
  unique()
```
...
test_00 %>%
  select(sched_dep_time_metric, op_diff1935) %>%
  ggplot() +
  geom_point(mapping = aes(x = sched_dep_time_metric, y = op_diff1935, color = 'Obs. prob. 2013'), size = 1) +
...
Before plotting the predicted values, I'll create a function that returns mean squared error^5^ and use it on the three prediction models
```{r, echo=FALSE, message=FALSE, warning=FALSE}
...
Looks like the bivariate model is the best. But let's plot all THREE
```{r echo=FALSE, message=FALSE, warning=FALSE}
ggplot() +
...
^5^ Mean squared error is equal to the average squared difference between the estimated values and what is estimated. That is, for each entry in the data set, it subtracts the value predicted by our model from the actual value, squares it, then sums the squared differences for all entries. In math notation, it's: $$MSE = \frac{1}{n} \sum_{i = 1}^{n} (actual_i - predicted_i)^{2}$$
...
test_17 <- flights_17 %>%
  ...
  mutate(diff_20h = if_else(sched_dep_time < 2000,
                            1960 - sched_dep_time,
                            sched_dep_time - 2000),
         sched_dep_time_metric = 100 * (trunc(sched_dep_time/100)) + 5/3 * as.numeric(str_sub(sched_dep_time, -2, -1)),
...
paste('sse ', signif(sse, 4), 'mse ', signif(mse, 4))
...
test_17 %>%
  select(sched_dep_time_metric, op_delay, pred_prob) %>%
  unique() %>%
  ggplot() +
...
Graph looks good too. Yes, I'm satisfied with this. Alternately I could update the model by replacing the 2013 data with 2017 data or combining them. More work for another day
...
dep_delay_skew %>%
  ggplot() +
  geom_line(aes(i, skew_rightward, color = 'skew by cutoff'), size = 1.5) +
  theme_fivethirtyeight() +
...
### Assuaging concerns with is_delay

In a report centered around delay, the limited analysis of the delay variable may have been a surprise. I'll address three concerns a reader might have with the methods I followed:

1. Why did you choose a binary variable that measured whether there was a delay and lose the value of knowing the minute measure of delay?
2. What if scheduled departure time is just a proxy for departure frequency?
3. Did missing values affect the results?

See above for what I believe sufficiently addresses #1. For #2, the data shows that the two variables, scheduled departure time and departure frequency, are not as correlated as one would think.
```{r message=FALSE, warning=FALSE, paged.print=FALSE}
flights %>%
  filter(!is.na(dep_delay)) %>%
  mutate(half = round(2 * (hour + minute / 60)) / 2) %>%
  group_by(half) %>%
  mutate(is_delay = if_else(dep_delay > 0, 1, 0)
         ,op_half = mean(is_delay)
         ,dep_half = n()
         ,sched_dep_time_metric = round((sched_dep_time / 100 - trunc(sched_dep_time / 100)) 
                                        * 500/3 + 100 * (trunc(sched_dep_time / 100)))) %>%
  ungroup() %>%
  mutate(dep_time_object = hm(paste
                              (trunc(sched_dep_time/100) ## This line provides the hour
                                ,str_sub(sched_dep_time,-2,-1))), ## This line provides the minute
  ...
  mutate(op_diff1935 = mean(is_delay, na.rm = T)) %>%
  ungroup() %>%
  select(sched_dep_time_metric, dep_half) %>%
  unique() %>%
  ggplot(aes(sched_dep_time_metric, dep_half, color = 'Departures per half hour')) +
  geom_point() +
  geom_smooth(aes(color = 'Smoothed departures per half hour')) +
  scale_color_fivethirtyeight(labels = c('Departures per half hour',
                                         'Smoothed departures per half hour'),
                              name = '') +
  ggtitle('Departure frequency is bimodal but departure delay was not')
```


For #3, I'll build a dataframe that separates into different groups entries where departure delay is NULL and entries where it is not
```{r}

temp_00 <- flights %>%
  mutate(delay_na = if_else(is.na(dep_delay), 'is na', 'is not na')
         ,dep_time_object = hm(paste
                              (trunc(sched_dep_time/100) ## This line provides the hour
                                ,str_sub(sched_dep_time,-2,-1))) ## This line provides the minute
         ...
         ,diff_1935h = abs(as.numeric(dep_time_object - hm('19:35')) / 60)) %>%
  group_by(sched_dep_time) %>%
  mutate(pred_prob = 1/(1+exp(-(0.279603750 - 0.001999519 * diff_1935h)))) %>%
  ungroup()
```
...
temp_00 %>%
  group_by(delay_na) %>%
  summarise(num = n()
            ,mean_prob = mean(pred_prob,na.rm = T)
            ,mdn_prob = median(pred_prob, na.rm = T)
            ,q_25 = quantile(pred_prob, probs = 0.25)
            ,q_75 = quantile(pred_prob, probs = 0.75))
```


It appears that entries where departure delay is NULL may be different from the entries where departure delay is numeric. The good news is that relatively few entries are NULL.

How different are the groups with respect to the density of predicted probability?
```{r}

ggplot(temp_00, mapping = aes(pred_prob)) +
  geom_density() +
  facet_wrap(~delay_na, ncol = 1) +
  theme_fivethirtyeight() +
  theme(axis.title = element_text()) +
  ylab('density') +
  xlab('predicted probability') +
  ggtitle('A bit different')


```


How different are the groups with respect to the density of scheduled departure time?
```{r}

ggplot(temp_00) +
  geom_density(aes(sched_dep_time_metric)) +
  facet_wrap(~delay_na, ncol = 1) +
  theme_fivethirtyeight() +
  theme(axis.title = element_text()) +
  ylab('density') +
  xlab('scheduled departure time') +
  ggtitle('Also a bit different')

```


So the question that really matters: How much could this affect the model? I'll test by supposing the missing entries either all had delays or all did not have delays
```{r echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}

test_01 <- flights %>%
  mutate(is_delay = case_when(dep_delay > 0 ~ 1
                              ,dep_delay < 0 ~ 0)
         ,sched_dep_time_metric = 100 * (trunc(sched_dep_time/100)) 
         + as.numeric(str_sub(as.character(sched_dep_time), -2, -1)) / 3 * 5
         ,na_1_is_delay = if_else(is.na(dep_delay), 1, is_delay)
         ,na_0_is_delay = if_else(is.na(dep_delay), 0, is_delay))

test_01 %>%
  ggplot() +
  geom_smooth(aes(sched_dep_time_metric, is_delay, color = 'remove na'), fill = 'pink') +
  geom_smooth(aes(sched_dep_time_metric, na_1_is_delay, color = 'treat na as delayed'), fill = 'lightgreen') +
  geom_smooth(aes(sched_dep_time_metric, na_0_is_delay, color = 'treat na as on time'), fill = 'lightblue') +
  ggtitle('Different but not too different') +
  theme_fivethirtyeight()
```



Visually, it doesn't appear to have changed by too much, but what about correlation?
```{r}
...
test_01 %>%
  mutate(dep_time_object = hm(paste
                              (trunc(sched_dep_time/100) ## This line provides the hour
                                ,str_sub(sched_dep_time,-2,-1))) ## This line provides the minute
         ,diff_1935h = abs(as.numeric(dep_time_object - hm('19:35')) / 60)) %>%
  summarise(cor_na_na = cor(diff_1935h, is_delay, use = 'pairwise.complete.obs')
         ,cor_na_1 = cor(diff_1935h, na_1_is_delay, use = 'pairwise.complete.obs')
         ,cor_na_0 = cor(diff_1935h, na_0_is_delay, use = 'pairwise.complete.obs'))
