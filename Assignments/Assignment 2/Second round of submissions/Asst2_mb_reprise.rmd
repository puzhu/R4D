...
ggplot(., aes(x = year, y = pop, color = "black")) + ## the color should be set outside the aes call otherwise it is mapped as a data
  geom_line()
```
4. Add the comma to the end of an operation not at the beginning.
5. What's the reason for not summarising the data for the first chart? You are essentially plotting the same point on top of each other.
...
test_b %>%
  ...
  mutate(pred_bivar = 1/(1+exp(-(0.279603750 - 0.001999519 * diff_1935h)))) %>% 
  group_by(diff_1935h) %>%
  summarise(nObs = n_distinct(sched_dep_time_metric)) %>% 
  filter(nObs > 1)
